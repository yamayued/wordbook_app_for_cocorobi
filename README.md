...
> ワークフローはリポジトリ直下の静的ファイル（`index.html`, `script.js`, `styles.css` など）をそのまま配信します。追加のビルド工程は不要です。リポジトリに `.nojekyll` ファイルを同梱しているため、GitHub Pages の Jekyll 処理を無効化し、静的アセットがそのまま公開されます。
